...
function rndrHome(){
  ...
  // Overall reflects ONLY modules with valid tracking (Solar + WTG, MW-weighted
  // ≈ 70.4 : 70.2). Land and BOP are not yet tracked at the detail level so
  // their progress is shown as "Coming Soon" rather than a misleading number.
  const _tot = Math.round((_sol*70.4 + _wtg*70.2) / (70.4 + 70.2));

  // ── ROW 1: SQUARE KPI CARDS ──────────────────────────────────────────────
  document.getElementById('home-kpi-row').innerHTML=[
    {label:'Overall Progress',val:_tot+'%',       sub:'Solar + WTG · MW-weighted',col:'var(--ac)',  icon:'continuumlogo.png',nav:'home',  tip:'Overall project progress (Solar + WTG, MW-weighted). Land & BOP are not yet tracked at the detail level.'},
    ...
    {label:'BOP Progress',    val:'Coming Soon',  sub:'Detail tracker WIP',col:'var(--bop)', icon:'bop.png',           nav:'bop',   tip:'Detailed BOP activity tracking is coming in a future release.'},
  ...
  renderHomeWorkTable();
  if (typeof renderLiveCharts === 'function') { try { renderLiveCharts(); } catch(e){ console.warn('[live-viz]', e); } }
