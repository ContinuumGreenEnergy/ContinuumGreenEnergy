...
function _buildPODForm(t){
  if(t==='s'){
    ...
    const actOpts=_podActOptionsGrouped(_solarActsList('ITC-1'))+
  ...
  if(t==='w'){
    ...
    const actOpts=_podActOptionsGrouped(_wtgActsList())+
  ...
  if(t==='b'){
    ...
    const actOpts=_podActOptionsGrouped(_bopActsList('33kV'))+
      '<option value="__other__">— Other (specify below) —</option>';
...
function updateSolarPODActs(itcId){
  ...
  sel.innerHTML=_podActOptionsGrouped(_solarActsList(itcId))+
    '<option value="__other__">— Other (specify below) —</option>';
...
function updateBOPPODActs(sec){
  const sel=document.getElementById('pf-act');if(!sel)return;
  sel.innerHTML=_podActOptionsGrouped(_bopActsList(sec))+
    '<option value="__other__">— Other (specify below) —</option>';
  const w=document.getElementById('pf-other-wrap');
  if(w) w.style.display='none';
}
...
function rndrMap(){
  ...
  setTimeout(()=>{
    ...
    // Simplified scheme: ROW = red, everything else = green.
