...
function podTab(t){
  ...
  el.innerHTML=`
    <div class="pnl" style="position:relative;">
      ...
      <div class="al al-i" style="margin-bottom:9px;">ℹ️ POD = Planned work for ${fDate===todayIso?'today':'<b>'+esc(fDate)+'</b>'} (no login to submit). Status updates (Not Started → WIP → Completed) require the authorised person to unlock edits.</div>
...
function _podEntryCard(t,x,canEdit){
  ...
  const lbl = (typeof _statusLabel === 'function') ? _statusLabel(status)
            : (status==='done'?'Completed':status==='wip'?'WIP':'Not Yet Started');
  ...
  const planned = +x.qty||0;
  const prog = +x.progress||0;
  const key = `'${x.date}','${x.id}'`;
  const sb=(st,txt)=>`<button class="pod-sbtn ${status===st?'on-'+st:''}" ${canEdit?'':'disabled style="opacity:.45;cursor:not-allowed;"'} onclick="podSetStatus(${key},'${st}')">${txt}</button>`;
  return `
    <div style="background:var(--card3);border:1px solid var(--b2);border-radius:8px;padding:10px;margin-bottom:7px;font-size:10px;" id="pod-card-${esc(x.id)}">
      ...
      <div class="mst">
        ...
        ${status==='wip'&&prog>0?`<div class="ms">Progress so far: <b style="color:var(--wn);">${prog}${planned?' / '+planned:''}</b></div>`:''}
        ${status==='done'?`<div class="ms">Final qty: <b style="color:var(--ok);">${prog||planned}</b></div>`:''}
      </div>
      ${status==='wip'&&planned?`<div class="wt-live-bar" style="margin-top:5px;"><div style="width:${Math.min(100,Math.round(prog/planned*100))}%"></div></div>`:''}
      ...
      ${x.remark?`<div style="color:var(--wn);margin-top:3px;font-size:9px;">💬 ${esc(x.remark)}${x.statusByName?` — <b>${esc(x.statusByName)}</b>`:''}</div>`:''}
      ...
      <div class="pod-status-btns">
        ${sb('nys','⏸ Not Yet Started')}${sb('wip','🟡 WIP')}${sb('done','✅ Completed')}
      </div>
      <div class="pod-inline-fields" id="pod-inline-${esc(x.id)}" style="display:${(status==='wip'||status==='done')&&canEdit?'flex':'none'};">
        <input type="number" min="0" step="any" id="pod-prog-${esc(x.id)}" placeholder="${status==='done'?'Final qty':'Progress qty'}" value="${prog||''}" style="width:110px;">
        <input type="text" id="pod-rem-${esc(x.id)}" placeholder="Remark (authorised person)" value="${esc(x.remark||'')}" style="flex:1;min-width:140px;">
        <button class="btn bts" style="font-size:9px;" onclick="podSaveProgress(${key})">💾 Save</button>
      </div>
    </div>`;
}

// Status buttons — write straight to Firebase; the realtime listener
// re-renders every device within ~1 second. No save button, no refresh.
async function podSetStatus(date,id,status){
  if(typeof auth!=='undefined' && !auth.canEdit()){
    if(typeof _requireUnlock==='function') return _requireUnlock(()=>podSetStatus(date,id,status));
    return showToast('🔒 Login required to update work status','er');
  }
  // When moving to WIP/Done, reveal inline fields first if values not yet entered
  const inline=document.getElementById('pod-inline-'+id);
  try{
    const patch={status};
    const progEl=document.getElementById('pod-prog-'+id);
    const remEl=document.getElementById('pod-rem-'+id);
    if(status!=='nys'){
      if(progEl&&progEl.value!=='') patch.progress=+progEl.value;
      if(remEl&&remEl.value!=='')   patch.remark=remEl.value;
    } else { patch.progress=0; }
    await dataApi.updatePodStatus(date,id,patch);
    if(inline) inline.style.display = status==='nys' ? 'none' : 'flex';
    showToast('✅ Status updated — syncing to all devices','ok');
  }catch(err){
    showToast('❌ '+(err.message||'Update failed'),'er');
  }
}

// Inline progress-quantity + remark save
async function podSaveProgress(date,id){
  if(typeof auth!=='undefined' && !auth.canEdit()){
    if(typeof _requireUnlock==='function') return _requireUnlock(()=>podSaveProgress(date,id));
    return showToast('🔒 Login required','er');
  }
  const prog=+(document.getElementById('pod-prog-'+id)?.value||0);
  const rem=document.getElementById('pod-rem-'+id)?.value||'';
  try{
    await dataApi.updatePodStatus(date,id,{progress:prog,remark:rem});
    showToast('✅ Progress saved & synced','ok');
  }catch(err){
    showToast('❌ '+(err.message||'Save failed'),'er');
  }
}
