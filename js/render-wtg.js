...
function _isWtgEditor(){
  return (typeof CU!=='undefined' && CU && (CU.role==='wtg' || CU.role==='all'));
}
...
function _wtgEditBanner(){
  if(_isWtgEditor()){
    return `<div class="al al-g" style="margin:0 0 8px 0;font-size:9px;">✅ Logged in as <b>${CU.role==='all'?'Site Manager':'WTG Manager'}</b> — date fields are editable.</div>`;
  }
  return `<div class="al al-w" style="margin:0 0 8px 0;font-size:9px;display:flex;align-items:center;justify-content:space-between;gap:8px;">
    <span>🔒 <b>View-only</b> — date fields are locked. Login as WTG Manager to edit.</span>
    <button class="btn btwt" style="font-size:9px;padding:3px 8px;" onclick="reqLogin('wtg',()=>{if(CV==='wtg'){rndrWtg();wTab(curWT);}})">🔑 Login as WTG Manager</button>
  </div>`;
}
