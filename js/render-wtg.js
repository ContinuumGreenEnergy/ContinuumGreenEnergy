...
function rndrWtg(){
  ...
  // ── Live farm visualisation (wtg-farm.js) — re-renders on every tick ──
  if (typeof renderWtgFarm === 'function') { try { renderWtgFarm(); } catch(e){ console.warn('[farm]', e); } }
...
function selectZeroPoint(containerId){
  ...
  // Default container = wtg right panel (legacy); 'zp-body' = standalone view
  const rp = document.getElementById(containerId || 'wtg-right-panel'); if(!rp) return;
  const ed = _isWtgEditor();

  // Reset tabs (only relevant when called inside the WTG view)
  document.querySelectorAll('#view-wtg .tab').forEach((x,i)=>x.classList.toggle('on',i===0));
  curWT = 0;
