...
(function(){
  ...
  async function applyWtgChanges(){
    ...
    try {
      // Targeted Firebase writes — every turbine's MW + structure is
      // persisted individually via dataApi (no bulk saveDB overwrite).
      if(typeof dataApi !== 'undefined' && dataApi.updateTurbine){
        for(const t of DB.wtg.turbines){
          try {
            await dataApi.updateTurbine(t.id, {
              mw: t.mw, status: t.status, lp: t.lp, pp: t.pp,
              civil: t.civil, mech: t.mech, uss: t.uss, sup: t.sup,
              vendor: t.vendor || ''
            });
          } catch(e){ console.warn('[pc] turbine write failed', t.id, e); }
        }
      }
      // Project-level meta (name / total MW / count) as leaf updates.
      if(typeof dataApi !== 'undefined' && dataApi.debouncedUpdate){
        dataApi.debouncedUpdate('wtg/meta/name',    name);
        dataApi.debouncedUpdate('wtg/meta/totalMW', mw);
        dataApi.debouncedUpdate('wtg/meta/count',   count);
      }
      if(typeof showToast === 'function') showToast('✅ WTG project updated & synced', 'ok');
    } catch(e){
  ...
  async function applySolChanges(){
    ...
    try {
      // Targeted Firebase writes — each ITC block is persisted via
      // dataApi.setSolarItc, and project meta goes to solar/meta.
      if(typeof dataApi !== 'undefined'){
        if(dataApi.setSolarItc){
          for(const [id, itc] of Object.entries(DB.solar.itcs || {})){
            try { await dataApi.setSolarItc(id, itc); } catch(e){ console.warn('[pc] itc write failed', id, e); }
          }
        }
        if(dataApi.updateSolarMeta){
          const itcMW = {};
          Object.entries(DB.solar.itcs || {}).forEach(([id, d]) => { itcMW[id] = d.mw; });
          try { await dataApi.updateSolarMeta({ totalMW: mw, itcMW }); } catch(e){}
        }
        if(dataApi.debouncedUpdate) dataApi.debouncedUpdate('solar/meta/name', name);
      }
      if(typeof showToast === 'function') showToast('✅ Solar project updated & synced', 'ok');
    } catch(e){
      if(typeof showToast === 'function') showToast('❌ Save failed: '+(e.message||e), 'er');
    }
