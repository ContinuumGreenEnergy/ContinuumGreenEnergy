...
(function (global) {
  ...
  function listenWtgKpiOverrides(cb) {
    const ref = fbDB.ref('wtg/kpiOverrides');
    const h = _safe(s => cb(s.exists() ? s.val() : null));
    ref.on('value', h);
    return _track(ref, 'value', h);
  }
  ...
  // Live cache of the most recent daily snapshots (last ~30 days). Drives the
  // real day-over-day "vs last" KPI deltas — so the trend reflects ACTUAL
  // change since the previous recorded day, not a fixed mock baseline.
  function listenSnapshots(cb, limit = 30) {
    const ref = fbDB.ref('snapshots').orderByKey().limitToLast(limit);
    const h = _safe(s => cb(s.exists() ? s.val() : {}));
    ref.on('value', h);
    _active.add({ ref, eventType: 'value', handler: h });
