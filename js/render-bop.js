...
function rndr33kv(sec){
  ...
  // ── Derive feeder data from DB.bopActs['33kv'] (the live data source) ──
  // Each feeder key holds an 8-element progress array aligned to bopActDefs['33kv'].
  const defs = DB.bopActDefs['33kv'];
  const acts33 = DB.bopActs['33kv'] || {};
  const feederMeta = {
    'Feeder-1 SPDC':  {label:'WTG F1',  col:'var(--wtg)', bg:'rgba(0,200,255,.08)',
                       route:['MBI-12','KDK-462','MKD-258','MKD-52','MKD-211','MKD-253','PSS']},
    'Feeder-1 SPSC':  {label:'WTG F1b', col:'#5c8fff',    bg:'rgba(92,143,255,.08)',
                       route:['MKD-253','MKD-211','PSS']},
    'Feeder-4 SPSC':  {label:'WTG F2',  col:'#7c4dff',    bg:'rgba(124,77,255,.08)',
                       route:['CDP-221','AMK-264','MOB-403','BDK-25','MOB-142','PSS']},
    'Feeder-3 Solar': {label:'Solar F1',col:'var(--sol)', bg:'rgba(255,170,0,.08)',
                       route:['ITC-1 Tap','ITC-2 Tap','ITC-3 Tap','PSS']},
  };
  // weighted % for one feeder's activity array
  const feederPct = (arr)=>{
    if(!arr||!arr.length) return 0;
    return Math.round(defs.reduce((s,d,i)=>s+(arr[i]||0)/100*d.w,0));
  };
  // index of an activity by partial name
  const actIdx = (name)=>defs.findIndex(d=>d.n.toLowerCase().includes(name.toLowerCase()));

  // Ensure pole-count store exists for every feeder (idempotent self-heal)
  if(!DB.bop33poles || typeof DB.bop33poles !== 'object') DB.bop33poles = {};
  Object.keys(feederMeta).forEach(k=>{
    if(!DB.bop33poles[k]) DB.bop33poles[k] = {total:0, done:0};
    if(typeof DB.bop33poles[k].total!=='number') DB.bop33poles[k].total=0;
    if(typeof DB.bop33poles[k].done !=='number') DB.bop33poles[k].done =0;
  });

  const FEEDERS = Object.keys(feederMeta).map(key=>{
    const arr = acts33[key] || new Array(defs.length).fill(0);
    const m = feederMeta[key];
    const poles = DB.bop33poles[key] || {total:0,done:0};
    return {
      key, id:key, label:m.label, col:m.col, bg:m.bg, route:m.route,
      arr,
      pct: feederPct(arr),
      row: arr[actIdx('ROW')]||0,
      fdn: arr[actIdx('Foundation')]||0,
      erection: arr[actIdx('Erection')]||0,
      stringing: arr[actIdx('Stringing')]||0,
      testing: arr[actIdx('Testing')]||0,
      polesTotal: poles.total,
      polesDone:  poles.done,
      polesPct:   poles.total>0 ? Math.round(poles.done/poles.total*100) : 0,
    };
  });
  ...
  el.innerHTML=`
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;align-items:start;">
      ...
      <div class="pnl">
        ...
        <!-- Pole-count row (total & done poles per feeder) -->
        ...
        ${[
          ...
          {label:'Pole Erection', key:'erection'},
          {label:'Stringing', key:'stringing'},
...
function open33ProgressForm(){
  ...
  document.getElementById('p-b').innerHTML=`<form onsubmit="submit33Progress(event)">
    ...
    <div style="border-top:1px solid var(--b1);margin:8px 0 6px;padding-top:8px;">
      <div style="font-size:9px;font-weight:700;color:var(--kv3);margin-bottom:5px;">⚡ Pole Count — this feeder</div>
      <div class="fr">
        <div class="fg"><label class="fl">Total Poles</label><input class="fi" id="p33-ptot" type="number" min="0" placeholder="e.g. 364"></div>
...
function _p33SyncCurrent(){
  ...
  // pole counts for this feeder
  const poles=(DB.bop33poles&&DB.bop33poles[fk])||{total:0,done:0};
  const tEl=document.getElementById('p33-ptot');
  const dEl=document.getElementById('p33-pdone');
  if(tEl) tEl.value=poles.total||0;
  if(dEl) dEl.value=poles.done||0;
}
...
async function submit33Progress(e){
  ...
  // Update pole counts for this feeder
  if(!DB.bop33poles) DB.bop33poles={};
  DB.bop33poles[fk]={total:pTot, done:Math.min(pDone, pTot||pDone)};

  // Persist to Firebase — stored per-feeder/per-activity index
  try{
    if(typeof dataApi!=='undefined' && dataApi.updateBopAct){
      // bopActs['33kv'] is keyed by feeder; store the whole feeder array
      await dataApi.updateBopAct(fk+'|'+ai, {done:pct, feeder:fk, actIdx:ai,
                                              arr:DB.bopActs['33kv'][fk]});
    }
    if(typeof dataApi!=='undefined' && dataApi.updateBop33Poles){
      await dataApi.updateBop33Poles(fk, DB.bop33poles[fk]);
  ...
  cov('pov');
  if(typeof showToast==='function') showToast('✅ 33kV '+fk+' · '+actName+' → '+pct+'% · Poles '+DB.bop33poles[fk].done+'/'+DB.bop33poles[fk].total,'ok');
      ...
      <div>
        <div class="pnl" style="margin-bottom:10px;">
          ...
          ${ACTS66.map((a,i)=>{
            ...
            return`<div style="display:flex;align-items:center;gap:8px;padding:7px 4px;border-bottom:1px solid var(--b1);" data-tt="${a.n}: ${pct}% complete">
              <div style="width:10px;height:10px;border-radius:2px;background:${a.col};flex-shrink:0;"></div>
              <div style="flex:1;min-width:0;">
                <div style="font-size:10px;font-weight:700;color:var(--t1);">${a.n} — <span style="color:${a.col};">${pct}%</span></div>
...
async function submit66Progress(e){
  ...
  try{
    if(typeof dataApi!=='undefined' && dataApi.updateBop66Act){
  ...
  cov('pov');
  if(typeof showToast==='function') showToast('✅ 66kV · '+actName+' → '+pct+'%','ok');
  if(typeof updateOverallBars==='function') updateOverallBars();
  rndrBopSec('66kv');
  _bopRefreshKpis();
}
...
async function submitPSSGSSProg(e,sec){
  ...
  try{
    if(sec==='pss' && typeof dataApi!=='undefined' && dataApi.updatePssAct){
      await dataApi.updatePssAct(act,{done:cum,wip:wip});
    } else if(sec==='gss' && typeof dataApi!=='undefined' && dataApi.updateGssAct){
  ...
  cov('pov');
  if(typeof showToast==='function') showToast('✅ '+sec.toUpperCase()+' · '+act+' updated','ok');
  if(typeof updateOverallBars==='function') updateOverallBars();
  rndrBopSec(sec);
  _bopRefreshKpis();
}
