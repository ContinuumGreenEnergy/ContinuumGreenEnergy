...
(function (global) {
  ...
  function _tickerEnsure() {
    ...
    // The carousel lives IN THE TOPBAR — a single rectangular box that
    // fills the unused space between the project title and the action
    // buttons, so live updates are always visible at the top of the page.
    const tb = document.getElementById('tb');
    const tbr = tb && tb.querySelector('.tbr');
    if (!tb || !tbr) return;
    const bar = el('div', { id: 'notif-ticker', 'data-tt': 'Live site updates — rotating carousel (hover to pause, click to open history)' },
