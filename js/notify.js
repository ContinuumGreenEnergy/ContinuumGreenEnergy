...
(function (global) {
  ...
  function _tickerItems() {
    const live = [...(_items.entries())]
      .filter(([id, rec]) => _visible(rec))
      .sort((a, b) => (b[1].ts || 0) - (a[1].ts || 0))
      .slice(0, TICKER_MAX);
    if (live.length) return live;
    // Default content: if no live notification records exist yet, synthesise
    // entries from recent POD submissions so the box always shows useful
    // detail (module · activity · submitted-by user) on first load.
    return _podFallbackItems();
  }

  // Build pseudo-notification entries from the most recent POD submissions
  // across all modules, so the header box has meaningful default content.
  function _podFallbackItems() {
    try {
      if (typeof DB === 'undefined' || !DB.pod) return [];
      const modMap = { s: 'solar', w: 'wtg', b: 'bop', l: 'land' };
      const out = [];
      Object.keys(modMap).forEach(k => {
        (DB.pod[k] || []).forEach(p => {
          out.push(['pod-' + k + '-' + (p.id || Math.random()), {
            module: modMap[k],
            action: 'POD submitted',
            desc: (p.activity || 'Activity') + (p.qty ? (' · qty ' + p.qty) : ''),
            byName: p.by || 'Site Engineer',
            ts: p.ts || Date.parse((p.date || '') + 'T' + (p.time || '00:00')) || Date.now(),
            _fallback: true
          }]);
        });
      });
      return out.sort((a, b) => (b[1].ts || 0) - (a[1].ts || 0)).slice(0, TICKER_MAX);
    } catch (e) { return []; }
  }

  function _tickerEnsure() {
    if (document.getElementById('notif-ticker')) return;
    // Structured live-notification BOX docked in the header, between the
    // project title and the manager/action buttons. Shows the latest update
    // as a small card: module badge + action/description on the top line,
    // and the submitting user + time on the second line. Rotates through
    // recent updates continuously (pauses on hover); the 🔔 opens the full
    // history panel.
    const tb = document.getElementById('tb');
    const tbr = tb && tb.querySelector('.tbr');
    if (!tb || !tbr) return;
    const bar = el('div', { id: 'notif-ticker', 'data-tt': 'Live notifications — latest site updates (click 🔔 for full history)' },
  ...
  function _tickerRender(instant, flash) {
    ...
    const items = _tickerItems();
    if (!items.length) {
      mount(slot, el('div', { class: 'nt-empty' },
        el('span', { class: 'nt-empty-l1' }, 'No live updates yet'),
        el('span', { class: 'nt-empty-l2' }, 'POD submissions, progress, HSE & blockers appear here')));
    ...
    const col = MOD_COLOR[(rec.module || 'general').toLowerCase()] || MOD_COLOR.general;
    const modLbl = MOD_LABEL[(rec.module || 'general').toLowerCase()] || rec.module;
    // Structured 2-line card: line 1 = badge + action + desc; line 2 = 👤 user · time
    const node = el('div', { class: 'nt-msg' + (flash ? ' flash' : '') },
      el('span', { class: 'nt-stripe', style: { background: col } }),
      el('div', { class: 'nt-body' },
        el('div', { class: 'nt-line1' },
          el('span', { class: 'nt-mod', style: { color: col, borderColor: col } }, modLbl),
          el('span', { class: 'nt-act' }, rec.action || ''),
          el('span', { class: 'nt-desc' }, rec.desc || '')),
        el('div', { class: 'nt-line2' },
          el('span', { class: 'nt-by' }, '👤 ' + (rec.byName || 'Anonymous')),
          el('span', { class: 'nt-time' }, _fmtTime(rec.ts)))));
  ...
  global.notify = { togglePanel, markRead, markAllRead,
    ...
    refreshTicker: () => { try { _tickerRender(true); } catch (e) {} } };
