...
function rndrSolar(){
  ...
  _pageLogoTR();
  // ── Live solar field visualisation (solar-field.js) — re-renders on every tick ──
  if (typeof renderSolarField === 'function') { try { renderSolarField(); } catch(e){ console.warn('[solar-field]', e); } }
  // Section nav
...
async function handleMapUpload(itcId,input){
  ...
  try {
    ...
    ITC_MAPS[itcId] = url;
    // Persist the map URL at solar/itcMaps/{itcId} so every device shows it.
    try { if (typeof fbDB !== 'undefined') fbDB.ref('solar/itcMaps/' + itcId).set(url); } catch(e){}
