...
function showSolActDetail(itcId,idx){
  ...
  const _isSolEditor = (typeof CU!=='undefined' && CU && (CU.role==='solar' || CU.role==='all'));
  const lockAttr  = _isSolEditor?'':'disabled';
  const lockStyle = _isSolEditor?'':'opacity:.55;cursor:not-allowed;';
  const banner = _isSolEditor
    ? `<div class="al al-g" style="margin:0 0 8px 0;font-size:9px;">✅ Logged in as <b>${CU.role==='all'?'Site Manager':'Solar In-charge'}</b> — sub-activities are editable.</div>`
...
async function updateSolSubScope(itcId,actIdx,newScope){
  if(!(typeof CU!=='undefined' && CU && (CU.role==='solar' || CU.role==='all'))){
    if(typeof showToast==='function')showToast('🔒 Solar In-charge login required','er');
...
async function handleMapUpload(itcId,input){
  ...
  input.value='';
  // Auth-gate: must be Solar In-charge or Site Manager
  if(!CU || (CU.role!=='solar' && CU.role!=='all' && !CU.isAdmin)){
    showToast('🔒 Solar In-charge or Site Manager only','er');
    return;
  }
