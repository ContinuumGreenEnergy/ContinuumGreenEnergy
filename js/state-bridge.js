...
(function (global) {
  ...
  function _renderNow() {
    clearTimeout(_renderTimer);
    _renderTimer = null;
    // Coalesce to one actual render per animation frame. Many listener events
    // (and any local optimistic render) can request a re-render in the same
    // tick; without this guard a single WTG edit could rebuild the heavy
    // 26-turbine view 2-3 times back-to-back, freezing input for a second.
    // requestAnimationFrame also yields to the browser so typing / clicking
    // stays responsive between updates.
    ...
    const run = () => {
      ...
      // Refresh the header notification box (its POD-fallback content depends
      // on DB.pod, which this mirror may have just updated).
      try { if (global.notify && notify.refreshTicker) notify.refreshTicker(); }
      catch(e){ /* non-fatal */ }
    };
  ...
  function _hydrate() {
    ...
    // ── Daily snapshots (last ~30 days) → global cache for real KPI deltas ──
    // Powers the day-over-day "vs last" trend on the home KPI cards. We keep
    // it in a plain global (global.SNAPSHOTS) rather than appState because it
    // is read-only reference data, not something the mirror writes back.
    if (realtime.listenSnapshots) {
      realtime.listenSnapshots(snaps => {
        global.SNAPSHOTS = snaps || {};
        // Re-render so the KPI deltas refresh when snapshots load/update.
        _scheduleRender();
      }, 30);
    }
