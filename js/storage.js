...
(function (global) {
  ...
  function _upload(path, file, onProgress) {
    _validate(file);
    // NOTE: this app uses a demo password gate, not Firebase Auth, so
    // uploads are permitted for anonymous users too (POD submission is
    // public by spec). Storage rules still enforce the 5 MB / image-only
    // limits server-side.
  ...
  function uploadHseImage(file, onProgress) {
    const id = ((auth.current() || {}).uid || 'anon') + '_' + Date.now() + '.' + _ext(file);
  ...
  /**
   * Generic sub-activity photo upload (Solar / WTG progress photos).
   * Path: subphotos/{module}/{ownerId}/{ts}.{ext}
   */
  function uploadSubPhoto(module, ownerId, file, onProgress) {
    const mod = String(module || 'misc').replace(/[^a-z0-9_-]/gi, '_').slice(0, 20);
    const own = String(ownerId || 'x').replace(/[^a-z0-9_-]/gi, '_').slice(0, 40);
    const id = Date.now() + '.' + _ext(file);
    return _upload('subphotos/' + mod + '/' + own + '/' + id, file, onProgress);
  }
  ...
  global.storage = { uploadHseImage, uploadItcMap, uploadPodImage, uploadSubPhoto, deleteByPath };
