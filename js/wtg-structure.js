...
// Internal section keys (pre/erection/post) are unchanged so stored
// turbine data and Firebase records remain compatible. Only DISPLAY
// labels are updated per project scope: Civil → Mechanical → Electrical.
const WTG_STRUCTURE = {
  'pre': {
    label:'Civil',
    icon:'🏗️',
...
// optional:true stages are NOT counted in pathway % unless the user enables them.
const WTG_LP_STAGES = [
  ...
  {key:'culvert',    n:'Culverts / Cross-Drainage',        optional:true},
  {key:'loadtest',   n:'Load Trial (Trailer Movement)',    optional:true},
  {key:'handover',   n:'Pathway Ready for Logistics',      optional:true},
];
const WTG_PP_STAGES = [
  ...
  {key:'culvert',    n:'Permanent Culverts & Drains',      optional:true},
  {key:'edge',       n:'Edge Protection / Shoulders'},
  {key:'surfacing',  n:'Surfacing / Black-topping'},
  {key:'qc',         n:'QC Inspection & Punch Closure'},
  {key:'handover',   n:'Permanent Pathway Handover',       optional:true},
];

function _makePathwayStages(defs){
  return defs.map(d => ({
    key:d.key, n:d.n, done:false, date:'', remarks:'',
    optional: !!d.optional,
    // optional stages start NOT enabled — user opts them in
    enabled: !d.optional
  }));
}
...
function wtgInitPathways(t){
  ...
  ['lp','pp'].forEach(which => {
    ...
    if(!t.pathways[which]){
      ...
      // Seed from the legacy boolean (only mandatory stages)
      if((which === 'lp' && t.lp) || (which === 'pp' && t.pp)){
        t.pathways[which].stages.forEach(s => { if(!s.optional) s.done = true; });
      }
    } else {
      ...
      t.pathways[which].stages = defs.map(d => {
        ...
        if(ex){
          ex.optional = !!d.optional;
          if(typeof ex.enabled !== 'boolean') ex.enabled = !d.optional;
          return ex;
        }
        return {key:d.key, n:d.n, done:false, date:'', remarks:'',
                optional:!!d.optional, enabled:!d.optional};
      });
      // keep names current
...
// % complete for one pathway — only counts ENABLED stages
// (optional stages the user hasn't switched on are excluded entirely).
function wtgPathwayPct(t, which){
  const pw = t.pathways && t.pathways[which];
  if(!pw || !pw.stages || !pw.stages.length) return 0;
  const active = pw.stages.filter(s => s.enabled !== false);
...
function _ensureCustomActs(){
  ...
  // actOrder[section] = [key,key,...]  custom activity-key sequence (optional)
  // subOrder[section.actKey] = [mergedIdx,...]  custom sub-activity sequence (optional)
  if(!DB.wtg.customActs.actOrder || typeof DB.wtg.customActs.actOrder !== 'object'){
    DB.wtg.customActs.actOrder = {};
  }
  if(!DB.wtg.customActs.subOrder || typeof DB.wtg.customActs.subOrder !== 'object'){
    DB.wtg.customActs.subOrder = {};
  }
  return DB.wtg.customActs;
}

// Apply a saved order array to a list keyed by `.key`. Unknown/new keys are
// appended in their natural order so nothing is ever lost.
function _applyOrder(list, orderKeys){
  if(!Array.isArray(orderKeys) || !orderKeys.length) return list;
  const byKey = {};
  list.forEach(it => { byKey[it.key] = it; });
  const out = [];
  orderKeys.forEach(k => { if(byKey[k]){ out.push(byKey[k]); delete byKey[k]; } });
  // append anything not in the saved order
  list.forEach(it => { if(byKey[it.key]) out.push(it); });
  return out;
}
...
//
// When `turbine` is passed, the list ALSO includes that turbine's LOCAL
// activities (t.localActs) and local extra sub-activities (t.localExtraSubs).
// Local items carry .isLocal:true and never appear on other turbines.
function wtgGetActivities(sectionKey, turbine){
  const def = WTG_STRUCTURE[sectionKey];
  if(!def) return [];
  const custom = _ensureCustomActs();
  const localSubs = (turbine && turbine.localExtraSubs) || {};
  let out = [];
  ...
  def.activities.forEach(a => {
    ...
    const lExtras = localSubs[extraKey] || [];
    let subs = a.subs.concat(extras.map(x => x.n)).concat(lExtras.map(x => x.n));
    let subsMeta = a.subs.map(n => ({n, isDefault:true}))
                  .concat(extras.map((x,i) => ({n:x.n, isDefault:false, customIdx:i})))
                  .concat(lExtras.map((x,i) => ({n:x.n, isDefault:false, isLocal:true, customIdx:i})));
    const so = custom.subOrder[extraKey];
    if(Array.isArray(so) && so.length){
      const pairs = subs.map((n,i)=>({n, meta:subsMeta[i]}));
      const reordered = _applyOrderIdx(pairs, so);
      subs = reordered.map(p=>p.n);
      subsMeta = reordered.map(p=>p.meta);
    }
    out.push({key:a.key, n:a.n, subs, subsMeta, isDefault:true});
  });
  // Global custom activities (deletable, on every turbine)
  (custom[sectionKey]||[]).forEach(a => {
    const extraKey = sectionKey + '.' + a.key;
    const lExtras = localSubs[extraKey] || [];
    let subs = (a.subs||[]).map(s => s.n).concat(lExtras.map(x => x.n));
    let subsMeta = (a.subs||[]).map((s,i) => ({n:s.n, isDefault:false, customIdx:i}))
                  .concat(lExtras.map((x,i) => ({n:x.n, isDefault:false, isLocal:true, customIdx:i})));
    const so = custom.subOrder[extraKey];
    if(Array.isArray(so) && so.length){
      const pairs = subs.map((n,i)=>({n, meta:subsMeta[i]}));
  ...
  // Per-turbine LOCAL activities (only when a turbine is supplied)
  if(turbine && Array.isArray(turbine.localActs && turbine.localActs[sectionKey])){
    turbine.localActs[sectionKey].forEach(a => {
      const extraKey = sectionKey + '.' + a.key;
      const lExtras = localSubs[extraKey] || [];
      const subs = (a.subs||[]).map(s => s.n).concat(lExtras.map(x => x.n));
      const subsMeta = (a.subs||[]).map((s,i) => ({n:s.n, isDefault:false, isLocal:true, customIdx:i}))
                    .concat(lExtras.map((x,i) => ({n:x.n, isDefault:false, isLocal:true, customIdx:i})));
      out.push({key:a.key, n:a.n, subs, subsMeta, isDefault:false, isLocal:true});
    });
  }
  // apply saved activity-order
  out = _applyOrder(out, custom.actOrder[sectionKey]);
  return out;
}

// Apply an index-based order to a plain list (used for sub-activities).
function _applyOrderIdx(list, orderIdx){
  if(!Array.isArray(orderIdx) || !orderIdx.length) return list;
  const used = new Set();
  const out = [];
  orderIdx.forEach(i => { if(i>=0 && i<list.length && !used.has(i)){ out.push(list[i]); used.add(i); } });
  list.forEach((it,i)=>{ if(!used.has(i)) out.push(it); });
  return out;
}

// ── Reorder activities / sub-activities (authorized users persist this) ──
// Move an activity up/down within its section. dir = -1 (up) | +1 (down).
function wtgMoveActivity(sectionKey, actKey, dir){
  const c = _ensureCustomActs(); if(!c) return false;
  const cur = wtgGetActivities(sectionKey).map(a => a.key);
  const i = cur.indexOf(actKey);
  if(i < 0) return false;
  const j = i + dir;
  if(j < 0 || j >= cur.length) return false;
  [cur[i], cur[j]] = [cur[j], cur[i]];
  c.actOrder[sectionKey] = cur;
  return true;
}

// Move a sub-activity up/down within an activity. mergedIdx is its current
// position in the merged list. dir = -1 | +1.
function wtgMoveSub(sectionKey, actKey, mergedIdx, dir){
  const c = _ensureCustomActs(); if(!c) return false;
  const acts = wtgGetActivities(sectionKey);
  const act = acts.find(a => a.key === actKey);
  if(!act) return false;
  const n = act.subs.length;
  const j = mergedIdx + dir;
  if(j < 0 || j >= n) return false;
  // Build current index sequence then swap
  const seq = act.subs.map((_,k)=>k);
  [seq[mergedIdx], seq[j]] = [seq[j], seq[mergedIdx]];
  c.subOrder[sectionKey + '.' + actKey] = seq;
  return true;
}
...
function _wtgMakeSub(name){
  return {
    n: name,
    pStart:'', pEnd:'', aStart:'', aEnd:'',
    status:'pending', responsible:'', remarks:'',
    delayReason:'None', photo:'',
    totalQty:0, doneQty:0, todayQty:0, unit:''   // quantity tracking
  };
}
...
function wtgInitActs(t){
  if(!t) return;
  if(!t.acts) t.acts = {};
  // ensure per-turbine local-activity containers exist
  if(!t.localActs || typeof t.localActs !== 'object') t.localActs = {};
  if(!t.localExtraSubs || typeof t.localExtraSubs !== 'object') t.localExtraSubs = {};
  ['pre','erection','post'].forEach(s => {
    if(!Array.isArray(t.localActs[s])) t.localActs[s] = [];
  });
  _ensureCustomActs();
    ...
    for(const act of acts){
      if(!t.acts[sectionKey][act.key]){
        t.acts[sectionKey][act.key] = {
          subs: act.subs.map(name => _wtgMakeSub(name))
        };
      } else {
        ...
        if(cur.length < want){
          for(let i=cur.length;i<want;i++) cur.push(_wtgMakeSub(act.subs[i]));
        ...
        // ensure names match + qty fields exist on legacy records
        t.acts[sectionKey][act.key].subs.forEach((s,i)=>{
          if(act.subs[i]) s.n = act.subs[i];
          if(typeof s.totalQty !== 'number') s.totalQty = 0;
          if(typeof s.doneQty  !== 'number') s.doneQty  = 0;
          if(typeof s.todayQty !== 'number') s.todayQty = 0;
...
// ── Per-turbine LOCAL activities/subs (only affect ONE turbine) ──────────
// Add a local activity to a single turbine. Returns key or null.
function wtgAddLocalActivity(turbine, sectionKey, name, subs){
  if(!turbine || !WTG_STRUCTURE[sectionKey]) return null;
  if(!name || !String(name).trim()) return null;
  if(!turbine.localActs) turbine.localActs = {};
  if(!Array.isArray(turbine.localActs[sectionKey])) turbine.localActs[sectionKey] = [];
  const key = _slug(name);
  const cleanSubs = (Array.isArray(subs) ? subs : [])
    .map(s => String(s||'').trim()).filter(Boolean).map(n => ({n}));
  turbine.localActs[sectionKey].push({key, n:String(name).trim(), subs:cleanSubs});
  wtgInitActs(turbine);
  return key;
}

// Remove a local activity from a single turbine.
function wtgRemoveLocalActivity(turbine, sectionKey, actKey){
  if(!turbine || !turbine.localActs || !Array.isArray(turbine.localActs[sectionKey])) return false;
  const before = turbine.localActs[sectionKey].length;
  turbine.localActs[sectionKey] = turbine.localActs[sectionKey].filter(a => a.key !== actKey);
  if(turbine.acts && turbine.acts[sectionKey]) delete turbine.acts[sectionKey][actKey];
  return turbine.localActs[sectionKey].length < before;
}

// Add a local sub-activity to ANY activity, on a single turbine.
function wtgAddLocalSub(turbine, sectionKey, actKey, subName){
  if(!turbine || !subName || !String(subName).trim()) return false;
  // If it's a turbine-local activity, append directly to its subs
  const localAct = turbine.localActs && Array.isArray(turbine.localActs[sectionKey])
    ? turbine.localActs[sectionKey].find(a => a.key === actKey) : null;
  if(localAct){
    if(!Array.isArray(localAct.subs)) localAct.subs = [];
    localAct.subs.push({n:String(subName).trim()});
  } else {
    // default or global-custom activity → store in per-turbine localExtraSubs
    if(!turbine.localExtraSubs) turbine.localExtraSubs = {};
    const ek = sectionKey + '.' + actKey;
    if(!Array.isArray(turbine.localExtraSubs[ek])) turbine.localExtraSubs[ek] = [];
    turbine.localExtraSubs[ek].push({n:String(subName).trim()});
  }
  wtgInitActs(turbine);
  return true;
}

// Remove a local sub-activity (default + global subs are untouched).
function wtgRemoveLocalSub(turbine, sectionKey, actKey, mergedIndex){
  if(!turbine) return false;
  const acts = wtgGetActivities(sectionKey, turbine);
  const act = acts.find(a => a.key === actKey); if(!act) return false;
  const meta = act.subsMeta[mergedIndex];
  if(!meta || !meta.isLocal) return false;  // only local subs deletable here
  const localAct = turbine.localActs && Array.isArray(turbine.localActs[sectionKey])
    ? turbine.localActs[sectionKey].find(a => a.key === actKey) : null;
  if(localAct){
    // local activity's own subs come first; figure if this index is one of them
    const ek = sectionKey + '.' + actKey;
    const ownCount = (localAct.subs||[]).length;
    if(meta.customIdx < ownCount){
      localAct.subs.splice(meta.customIdx, 1);
    } else if(turbine.localExtraSubs && turbine.localExtraSubs[ek]){
      turbine.localExtraSubs[ek].splice(meta.customIdx - ownCount, 1);
    }
  } else {
    const ek = sectionKey + '.' + actKey;
    if(turbine.localExtraSubs && Array.isArray(turbine.localExtraSubs[ek])){
      turbine.localExtraSubs[ek].splice(meta.customIdx, 1);
    }
  }
  wtgInitActs(turbine);
  return true;
...
window.wtgMoveActivity = wtgMoveActivity;
window.wtgMoveSub = wtgMoveSub;
window.wtgAddCustomActivity = wtgAddCustomActivity;
window.wtgAddLocalActivity = wtgAddLocalActivity;
window.wtgRemoveLocalActivity = wtgRemoveLocalActivity;
window.wtgAddLocalSub = wtgAddLocalSub;
window.wtgRemoveLocalSub = wtgRemoveLocalSub;
