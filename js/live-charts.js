...
(function (global) {
  ...
  function renderLiveCharts() {
    ...
    _overlay('lv-cum-empty', !datasets.length);
    if (datasets.length) {
    ...
    } else { _destroy('ch-live-cum'); }

    // ── 2. Daily activity bars — submitted vs completed (7 days) ──
    // Sources: /pod entries (per their date key) + /dailyProgress entries
    // (per their server timestamp). Both load on boot — POD via the
    // 8-day prefetch in state-bridge, dailyProgress via its listener —
    // so the bars populate as soon as Firebase syncs, and re-draw live.
    ...
    const dailyHasData = submitted.some(v => v) || completed.some(v => v) || dpPerDay.some(v => v);
    ...
    // ── 3. Manpower trend — Σ mp per day (mp coerced: forms may store strings) ──
    const mpPerDay = days.map(d => pods.filter(p => p.date === d)
      .reduce((s, p) => s + (parseFloat(p.mp) || 0), 0));
    const mpHasData = mpPerDay.some(v => v > 0);
